...
PENJAWABAN = Kredensial(nama="penjawaban", baca=frozenset({Area.KORPUS}), tulis=frozenset(), indeks=frozenset(), tulis_indeks=frozenset())
PEMANGGIL_LLM = Kredensial(nama="llm", baca=frozenset({Area.KORPUS}), tulis=frozenset(), indeks=frozenset(), tulis_indeks=frozenset())
...
def test_pemisahan_penanda_status_bukan_kredensial_ditolak(tmp_path: Path) -> None:
    ...
    rusak = KREDENSIAL_BERSIH.replace(
        ...
        "tulis=frozenset(), indeks=frozenset(), tulis_indeks=frozenset())\nPEMANGGIL_LLM",
    )
    assert periksa_pemisahan_penyimpanan(_pohon(tmp_path, rusak))
