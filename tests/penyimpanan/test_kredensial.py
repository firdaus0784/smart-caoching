...
def test_kredensial_membawa_dua_himpunan() -> None:
    k = Kredensial(nama="uji", baca=frozenset({Area.KORPUS}), tulis=frozenset())
...
def test_himpunan_kosong_berarti_tidak_boleh_apa_pun() -> None:
    ...
    k = Kredensial(nama="kosong", baca=frozenset(), tulis=frozenset())
    for area in Area:
...
def test_menulis_tidak_menyiratkan_membaca() -> None:
    k = Kredensial(nama="uji", baca=frozenset(), tulis=frozenset({Area.KARANTINA}))
...
def test_nama_wajib_terisi() -> None:
    ...
    with pytest.raises(ValidationError):
        Kredensial(nama="", baca=frozenset(), tulis=frozenset())
